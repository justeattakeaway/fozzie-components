...
* Treat components on a case by case basis. Smaller components with lots of variation, like `f-button`, **should** have visual tests to check their many different sizes and colours. Alternatively, components with only one or two variations, such as `f-popover`, would not require these checks, since they are likely already being tested in a parent component.
