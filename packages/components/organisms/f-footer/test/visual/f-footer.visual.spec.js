...
devices.forEach(device => {
    describe(`f-footer - ${device} Visual Tests`, () => {
        locales = [
            'en-GB',
            'en-AU',
            'en-IE',
        ...
        locales.forEach(locale => {
            it(`should display the base footer for tenant: "${locale}"`, async () => {
                ...
                // Assert
                await browser.percyScreenshot(`f-footer - Base - ${locale}`, device);
            });
            ...
            it(`should display the footer with country selector for tenant: "${locale}"`, async () => {
                ...
                // Assert
                await browser.percyScreenshot(`f-footer - Country Selector - ${locale}`, device);
            });
            ...
            it(`should display slim footer when no content links are provided for tenant: "${locale}"`, async () => {
                ...
                // Assert
                await browser.percyScreenshot(`f-footer - Slim Footer - ${locale}`, device);
            });
        });
        ...
        locales.forEach(locale => {
            ...
            it(`should display the footer with courier links and country selector for tenant: "${locale}"`, async () => {
                ...
                // Assert
                await browser.percyScreenshot(`f-footer - Courier Links and Country Selector - ${locale}`, device);
            });
        });
    });
});
